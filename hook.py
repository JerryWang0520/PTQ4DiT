...
def hook_integer_compute(module, input, output, reuse_type=None):
    ...
    ###################### Integer Domain Computing ################################
    ###### 0. integer computing
    if reuse_type == None:
        out_int = module.fwd_func(input_int - za, weight_int - zw, **module.fwd_kwargs)

    ###### 1. spatial diff computing (only support linear layer)
    elif reuse_type == 'spatial':
    ...
    ###### 2. temporal diff computing
    elif reuse_type == 'temporal':
    ...
    ###### 4. undefined reuse types
    else:
        raise Exception("Unsupported reuse_type")
    #######################################

    # rescale output to float domain
